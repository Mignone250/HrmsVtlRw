...
$sql="UPDATE user_registration SET FIRST_NAME='$first_name',LAST_NAME='$last_name',GENDER='$gender',NATIONAL_ID='$NationalID',
											PHONE_NUMBER='$phone',EMAIL='$email',DISTRICT='$district',POSITION = '$position',DEPARTMENT='$department',ADDITIONAL_ROLE='$ADDITIONAL_ROLE',USERNAME='$username',USER_TYPE='$user_type'
											WHERE USER_ID = '$ID'"; 
...
$sql = "SELECT * FROM user_registration WHERE USER_ID = '$ID'"; 
					...
					if ($result->num_rows > 0) {
						...
						while($row = $result->fetch_assoc()) {
						  ...
						  <div class="bio-row">
                            <p><span><strong>ADDITIONAL ROLE</strong></span>: <?php  echo $ADDITIONAL_ROLE;  ?></p>
                          </div>
...
$sql = "SELECT * FROM user_registration WHERE USER_ID = '$ID'"; 
					...
					if ($result->num_rows > 0) {
					   ...
					   <div class="form-group">
                        <label class="col-lg-2 control-label">Additional Role</label>
                        <div class="col-lg-6">
                          <select class="form-control" name="ADDITIONAL_ROLE">
                                                  <option><?php  echo $ADDITIONAL_ROLE;  ?></option>
                                                  <option>HOD(Head Of Department)</option>
                                                  <option>HOA(Head Of Academics)</option>
